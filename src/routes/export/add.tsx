...
const AddExportPage: React.FC = () => {
  const [formData, setFormData] = useState<FormData>({
    ...
    createdDate: '',
    products: [
      { productId: '', productName: '', unit: '', quantity: 0, unitPrice: 0, totalPrice: 0 },
    ],
  });
  ...
  const handleProductChange = (index: number, field: string, value: string | number) => {
    ...
    updatedProducts[index].totalPrice = (Number(updatedProducts[index].quantity) || 0) * (Number(updatedProducts[index].unitPrice) || 0);
  ...
  const addProduct = () => {
    setFormData({
      ...formData,
      products: [...formData.products, { productId: '', productName: '', unit: '', quantity: 0, unitPrice: 0, totalPrice: 0 }],
    });
  };
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-8 border-2 border-blue-100">
      <h1 className="text-3xl font-extrabold text-blue-800 mb-8 drop-shadow uppercase tracking-wide">Tạo phiếu xuất</h1>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <h2 className="text-xl font-bold text-gray-800">Danh sách sản phẩm xuất</h2>
